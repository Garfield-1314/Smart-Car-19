...
/*【全向时调用】*/
		...
		||((flap[0]==1||flap[0]==2||flap[0]==3)&&pic_flag.ramp_flag!=0&&pic_flag.cross_all_flag==0&&pic_flag.round_left_flag==0&&pic_flag.round_right_flag==0))
	{
		...
		if(car_system.Vw[0]<=-125)
			car_system.Vw[0]=-125;
		else if(car_system.Vw[0]>=125)
				car_system.Vw[0]=125;
		
		if(car_system.Vw[1]<=-125)
				car_system.Vw[1]=-125;
		else if(car_system.Vw[1]>=125)
				car_system.Vw[1]=125;

		if(car_system.Vw[2]<=-125)
				car_system.Vw[2]=-125;
		else if(car_system.Vw[2]>=125)
				car_system.Vw[2]=125;

		if(car_system.Vw[3]<=-125)
				car_system.Vw[3]=-125;
		else if(car_system.Vw[3]>=125)
				car_system.Vw[3]=125;
	}
/**************************************************************************************/
/**************************************************************************************/
	...
	if(car_system.Vw[0]<=-200)
			car_system.Vw[0]=-200;
	else if(car_system.Vw[0]>=200)
			car_system.Vw[0]=200;
	
	if(car_system.Vw[1]<=-200)
			car_system.Vw[1]=-200;
	else if(car_system.Vw[1]>=200)
			car_system.Vw[1]=200;

	if(car_system.Vw[2]<=-200)
			car_system.Vw[2]=-200;
	else if(car_system.Vw[2]>200)
			car_system.Vw[2]=200;

	if(car_system.Vw[3]<=-200)
			car_system.Vw[3]=-200;
	else if(car_system.Vw[3]>=200)
			car_system.Vw[3]=200;
}
...
void car_attitude()
{
	...
	if(((flap[0]==1)&&pic_flag.cross_all_flag==0&&pic_flag.round_left_flag!=0&&pic_flag.round_right_flag==0)
		...
		||(flap[0]==8&&pic_flag.endline_flag!=0)
		||flap[0]==0||flap[0]==17)
	{
		race_speed_solution();
	}
...
{
	flag.keep_omega=OK;	
	if(car_system.set_Omega+2>=car_system.Omega&&car_system.Omega>=car_system.set_Omega-2)
	{
		system_delay_ms(50);
		if(car_system.set_Omega+2>=car_system.Omega&&car_system.Omega>=car_system.set_Omega-2)
		{
...
void race_speed_solution()/*【赛道行驶PID】*/
{
	...
	d_speed=abs((int16)(t_speed_m*1.5));
	car_system.Vw[0]=(float)(car_system.Vw[0]-d_speed-t_speed_m);
	car_system.Vw[2]=(float)(car_system.Vw[2]-d_speed-t_speed_m);
	
	car_system.Vw[1]=(float)(car_system.Vw[1]-d_speed+t_speed_m);
	car_system.Vw[3]=(float)(car_system.Vw[3]-d_speed+t_speed_m);
}
