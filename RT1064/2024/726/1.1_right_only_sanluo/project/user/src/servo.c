...
void servo_a_put()
{
	...
	{
		system_delay_ms(200);
		servo.pwm[0]=690;servo.pwm[1]=960;servo.pwm[2]=493;servo.pwm[3]=frame1.frame_pwm[frame1.at_frame];	
		system_delay_ms(400);
		flag.citie=OK;
		system_delay_ms(700);	
		servo.pwm[0]=690;servo.pwm[1]=900;servo.pwm[2]=520;servo.pwm[3]=frame1.frame_pwm[frame1.at_frame];	
		system_delay_ms(300);	
		servo.pwm[0]=770;servo.pwm[1]=760;servo.pwm[2]=460;servo.pwm[3]=frame1.frame_pwm[frame1.at_frame];	
		system_delay_ms(500);
...
{
//	servo.flag[0]=0;
	if(servo.pwm_set[3]<servo.pwm[3]-20||servo.pwm_set[3]>servo.pwm[3]+20)
	{
		if(servo.pwm_set[3]<servo.pwm[3]&&servo.pwm_set[3]!=0)
		{
			if(flap[0]==7)
				servo.pwm_set[3]+=5;
			else
		...
		else if(servo.pwm_set[3]>servo.pwm[3]&&servo.pwm_set[3]!=0)
		{
			if(flap[0]==7)
				servo.pwm_set[3]-=5;
			else
	...
	else if(servo.pwm_set[3]>servo.pwm[3]-50||servo.pwm_set[3]<servo.pwm[3]+50)
	{
		if(servo.pwm_set[3]<servo.pwm[3]&&servo.pwm_set[3]!=0)
		{
			servo.pwm_set[3]+=1;
	...
	if(servo.pwm_set[3]==servo.pwm[3]&&(flap[0]==7||flap[0]==16||flap[0]==5))
	{
//		servo.pwm_set[3]=servo.pwm[3];
		servo.flag[0]++;
	}
	else
	{
	...
	servo_give(servo.pwm[0],servo.pwm[1],servo.pwm[2],servo.pwm_set[3]);
}