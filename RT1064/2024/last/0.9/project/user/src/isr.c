...
void PIT_IRQHandler(void)
{
    ...
    if(pit_flag_get(PIT_CH1))
    {
        pit_flag_clear(PIT_CH1);
			if(flag.citie==OK)
				gpio_set_level(citie,GPIO_HIGH);
			else
				gpio_set_level(citie,GPIO_LOW);
			
			servo_give(servo.pwm[0],servo.pwm[1],servo.pwm[2],servo.pwm[3]);
    }
    ...
    if(pit_flag_get(PIT_CH3))
    {
			...
			sys_time();
			card_identify_time_solution();
			cross_round_card_over_solution();
			obstacle_time_solution();
//			Scattered_card_time_solution();
    }
...
void LPUART4_IRQHandler(void)
{
    ...
    {
        // 接收中断 
			openart2_uart_rx_interrupt_handler();
