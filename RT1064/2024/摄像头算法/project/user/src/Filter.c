...
float lastVal2;//上次计算值
float rcHpFiter2(float val,float k)
{
    lastVal2=((float)val*k+lastVal2*(1-k));
    //    return -(val-rcPara->lVal);//滤波结果
    //return -(val-lVal);//如果直接返回滤波结果，滤波后图像是倒转的，在心电图等一些场合，需要将图像再镜像过来
	return lastVal2;
}

float lastVal3;//上次计算值
float rcHpFiter3(float val,float k)
{
    lastVal3=((float)val*k+lastVal3*(1-k));
    //    return -(val-rcPara->lVal);//滤波结果
    //return -(val-lVal);//如果直接返回滤波结果，滤波后图像是倒转的，在心电图等一些场合，需要将图像再镜像过来
	return lastVal3;
}
...
float Sliding_Window_Filtering2(int data_channel, float data_stream)
{
	...
	/* ???????? */
	static float Filter_Window2[Channel_Number][Window_Size] = {0};
	
	/* ?????????? */
	for(int i = 0; i < Window_Size-1; i++){
		Filter_Window2[data_channel][i] = Filter_Window2[data_channel][i+1];	// ?????
	}
	Filter_Window2[data_channel][Window_Size-1] = data_stream;	// ?????

	/* ?????? */
	float Sum = 0;
	for(int i = 0; i < Window_Size; i++){
		Sum += Filter_Window2[data_channel][i];
	}
...
float Sliding_Window_Filtering3(int data_channel, float data_stream)
{
	...
	/* ???????? */
	static float Filter_Window3[Channel_Number][Window_Size] = {0};
	
	/* ?????????? */
	for(int i = 0; i < Window_Size-1; i++){
		Filter_Window3[data_channel][i] = Filter_Window3[data_channel][i+1];	// ?????
	}
	Filter_Window3[data_channel][Window_Size-1] = data_stream;	// ?????

	/* ?????? */
	float Sum = 0;
	for(int i = 0; i < Window_Size; i++){
		Sum += Filter_Window3[data_channel][i];
	}
