...
void PIT_IRQHandler(void)
{
    ...
    { 
			...
			get_imu963ra_data();
			if((flag1.car_move==1||flag[1]==1||flag[1]==4)&&(gpio.key[3]==1))
			{
				get_encoder();
//				car_move_dot();
//				speed_control();
    ...
    if(pit_flag_get(PIT_CH2))
    {
        pit_flag_clear(PIT_CH2);
				stime++;
//			printf("%f\t%f\t%f\t%f\t\r\n",imu963ra.dx,imu963ra.dy,imu963ra.vx,imu963ra.vy);
			printf("%d\t%d\t%d\t%d\r\n",color[0],color[1],rcx,rcy);
//				ano7(imu963ra.acc_x,imu963ra.acc_y,0,0);
		}
...
void LPUART1_IRQHandler(void)
{
    if(kLPUART_RxDataRegFullFlag & LPUART_GetStatusFlags(LPUART1))
    {
        // 接收中断
			...
			openart1_uart_rx_interrupt_handler();
    }
