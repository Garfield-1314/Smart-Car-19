...
void dis_pid_init()
{
	pid.Kp[4]=4;  pid.Ki[4]=0;  pid.Kd[4]=0;//x方向
	pid.Kp[5]=4;  pid.Ki[5]=0;  pid.Kd[5]=0;//y方向

	pid.Kp[6]=2;  pid.Ki[6]=0;  pid.Kd[6]=0;//陀螺仪
	
	pid.Kp[7]=0.35;  pid.Ki[5]=0;  pid.Kd[5]=0;
	pid.Kp[8]=0.35;  pid.Ki[5]=0;  pid.Kd[5]=0;
	
	pid.Kp[9]=4;  pid.Ki[9]=0;  pid.Kd[9]=0;
}
...
void motor_text()
{
	...
	pwm_set_duty(PWM1_MODULE0_CHB_D13,000);
	pwm_set_duty(PWM1_MODULE0_CHA_D12,000);
	pwm_set_duty(PWM2_MODULE3_CHB_D3,3000);
	pwm_set_duty(PWM2_MODULE3_CHA_D2,3000);
}

void race_speed_solution()/*【赛道行驶PID】*/
{
		
	pid.speed=car_system.speed;
	t_speed_l=-PID_dis(9,33,center_t);
	t_speed_r=PID_dis(9,33,center_t);
	
	car_system.Vw[0]=car_system.Vw[2]=(float)(pid.speed+t_speed_l);
	car_system.Vw[1]=car_system.Vw[3]=(float)(pid.speed+t_speed_r);
	flag1.race_move=0;
}